...
// If deterministic algorithms are enabled, fill the tensor with NaN if it is
// floating point or complex type, or fill with max value if it is integer type
inline Tensor& maybe_fill_empty_deterministic(Tensor& tensor) {
  if (!at::globalContext().deterministicAlgorithms()) {
    return tensor;
  }
  if (tensor.is_floating_point() || tensor.is_complex()) {
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      kBFloat16, kHalf, tensor.scalar_type(), "maybe_fill_empty_deterministic", [&]() {
        tensor.fill_(std::numeric_limits<scalar_t>::quiet_NaN());
    });
  } else {
    AT_DISPATCH_INTEGRAL_TYPES_AND(
      kBool, tensor.scalar_type(), "maybe_fill_empty_deterministic", [&]() {
